...
class WBlockElementZapper {
  ...
  init() {
    ...
    console.log("Syferlab Element Zapper initialized");
  }
  ...
  generateCandidatesForElement(element) {
    ...
    if (parent && parent !== document.body) {
      if (parent.className) {
        ...
        if (parentClass) {
          candidates.push(
            `.${CSS.escape(parentClass)} ${element.tagName.toLowerCase()}`
          );
        }
      }
    }
  ...
  updateCandidatesList() {
    ...
    this.candidates.forEach((candidate, index) => {
      ...
      item.className = `candidate-item ${
        index === this.selectedCandidateIndex ? "selected" : ""
      }`;
  ...
  applyLoadedRules(rules) {
    ...
    console.log(
      `Applied ${rules.length} existing zapper rules for ${location.hostname}`
    );
  }
  ...
  quit() {
    ...
    console.log("Syferlab Element Zapper deactivated");
  }
}
