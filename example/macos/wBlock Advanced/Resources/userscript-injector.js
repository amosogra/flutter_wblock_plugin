  ...
  class UserScriptEngine {
    ...
    setupDocumentEventListeners() {
      ...
      if (document.readyState === "loading") {
        console.log(
          "[wBlock] Document is loading, setting up event listeners for ready states."
        );

        document.addEventListener("DOMContentLoaded", () => {
          console.log(
            "[wBlock] DOMContentLoaded event fired, retrying pending scripts."
          );
        ...
        window.addEventListener("load", () => {
          console.log(
            "[wBlock] Window load event fired, retrying pending scripts."
          );
          this.retryPendingScripts();
        });
      } else {
        console.log(
          `[wBlock] Document already ready (${document.readyState}), no need for event listeners.`
        );
      }
    }
    ...
    retryPendingScripts() {
      ...
      console.log(
        `[wBlock] Retrying ${this.pendingScripts.length} pending scripts...`
      );
    ...
    requestUserScripts() {
      ...
      console.log(
        `[wBlock] Requesting userscripts for URL: ${window.location.href} with requestId: ${requestId}`
      );
      ...
      if (
        ...
        console.log(
          "[wBlock] Sending requestUserScripts via browser.runtime.sendMessage"
        );
        browser.runtime
          .sendMessage(messagePayload, (response) => {
            if (browser.runtime.lastError) {
              console.error(
                "[wBlock] Error sending message to native via browser.runtime.sendMessage:",
                browser.runtime.lastError
              );
            } else {
              console.log(
                "[wBlock] browser.runtime.sendMessage response (if any):",
                response
              );
            }
          })
          .catch((error) => {
            console.error(
              "[wBlock] Failed to send message via browser.runtime.sendMessage:",
              error
            );
          });
      } else if (
        ...
        console.log(
          "[wBlock] Sending requestUserScripts via safari.extension.dispatchMessage"
        );
        ...
        console.error(
          "[wBlock] No suitable messaging API found for sending requestUserScripts."
        );
      }
    }
      ...
      if (
        ...
        browser.runtime.onMessage.addListener(
          (message, sender, sendResponse) => {
            console.log(
              "[wBlock] Received message via browser.runtime.onMessage:",
            ...
            if (scriptsToInject) {
              console.log(
                "[wBlock] Extracted userscripts from message (browser.runtime.onMessage):",
                scriptsToInject
              );
              this.injectUserScripts(scriptsToInject);
            } else {
              console.log(
                "[wBlock] No userscripts found in received message (browser.runtime.onMessage)."
              );
            }
          }
        );
        ...
        console.log(
          "[wBlock] Using safari.self.addEventListener for listening."
        );
        safari.self.addEventListener(
          "message",
          (event) => {
            console.log(
              "[wBlock] Received message via safari.self.addEventListener:",
              event.name,
              JSON.parse(JSON.stringify(event.message || {}))
            );
            let scriptsToInject = null;
            if (
              event.name === "requestUserScripts" &&
              event.message &&
              event.message.userScripts
            ) {
              scriptsToInject = event.message.userScripts;
            } else if (
              event.name === "requestRules" &&
              event.message &&
              event.message.payload &&
              event.message.payload.userScripts
            ) {
            ...
            if (scriptsToInject) {
              console.log(
                "[wBlock] Extracted userscripts from message (safari.self.addEventListener):",
                scriptsToInject
              );
              this.injectUserScripts(scriptsToInject);
            } else {
              console.log(
                "[wBlock] No userscripts found in received message (safari.self.addEventListener)."
              );
            }
          },
          false
    ...
    injectUserScripts(userScripts) {
      ...
      if (!Array.isArray(userScripts)) {
        console.warn(
          "[wBlock] injectUserScripts called with non-array:",
          userScripts
        );
        return;
      }
    ...
    injectUserScript(script) {
      if (!script || !script.name) {
        console.warn(
          "[wBlock] Attempted to inject invalid script object:",
          script
        );
        return;
      }
      ...
      if (this.injectedScripts.has(script.name)) {
        console.log(
          `[wBlock] Userscript ${script.name} already injected. Skipping.`
        );
        return;
      }
      ...
      try {
        if (!this.shouldRunScript(script)) {
          ...
          if (!this.pendingScripts.some((s) => s.name === script.name)) {
            console.log(
              `[wBlock] Adding ${script.name} to pending scripts list.`
            );
        ...
        console.log(
          `[wBlock] Appended <script> tag for ${script.name} to the DOM.`
        );

        this.injectedScripts.add(script.name);
        console.log(
          `[wBlock] Successfully injected and registered userscript: ${
            script.name
          } at ${script.runAt || "document-end"}`
        );
      } catch (error) {
        console.error(
          `[wBlock] Failed to inject userscript ${script.name}:`,
          error
        );
      }
    }
    ...
    shouldRunScript(script) {
      ...
      console.log(
        `[wBlock] Checking shouldRunScript for ${script.name}: runAt='${runAt}', document.readyState='${readyState}'`
      );
      ...
      switch (runAt) {
        ...
        default:
          console.warn(
            `[wBlock] Unknown runAt value: '${runAt}' for script ${script.name}. Defaulting to document-end behavior.`
          );
          shouldRun = readyState === "interactive" || readyState === "complete";
          break;
      }
      ...
      if (!shouldRun) {
        console.log(
          `[wBlock] Userscript ${script.name} will NOT run at this time (runAt: '${runAt}', readyState: '${readyState}').`
        );
      } else {
        console.log(
          `[wBlock] Userscript ${script.name} WILL run at this time (runAt: '${runAt}', readyState: '${readyState}').`
        );
      }
      return shouldRun;
    }
...
(function() {
    ...
    const GM = {
        info: {
            ...
            scriptHandler: 'Syferlab Injector',
            version: '0.1.1' // Incremented version for this logging update
        },
        log: function(...args) { console.log('[UserScript:${
          script.name
        }]', ...args); }
    ...
    try {
        ...
        console.error('[wBlock UserScript Execution Error] in ${
          script.name
        }:', error);
    }
})();
  ...
  if (document.documentElement) {
    console.log(
      "[wBlock] document.documentElement exists, creating UserScriptEngine instance."
    );
    new UserScriptEngine();
  } else {
    console.log(
      "[wBlock] document.documentElement does not exist, deferring UserScriptEngine instance creation to DOMContentLoaded."
    );
    document.addEventListener("DOMContentLoaded", () => {
      console.log(
        "[wBlock] DOMContentLoaded fired, creating UserScriptEngine instance."
      );
      new UserScriptEngine();
    });
  }
